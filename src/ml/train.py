...
def train_model():
    spark = SparkSession.builder.appName("TrainLogNoiseFilter").getOrCreate()
    ...
    logger.info(
        f"Data split: {train_df.count()} training rows, {test_df.count()} test rows."
    )

    indexer1 = StringIndexer(
        inputCol="event_type", outputCol="event_type_idx", handleInvalid="keep"
    )
    indexer2 = StringIndexer(
        inputCol="action", outputCol="action_idx", handleInvalid="keep"
    )
    indexer3 = StringIndexer(
        inputCol="severity", outputCol="severity_idx", handleInvalid="keep"
    )
    ...
    rf = RandomForestClassifier(
        labelCol="is_anomaly", featuresCol="features", numTrees=20, maxDepth=5
    )
    ...
    evaluator_f1 = MulticlassClassificationEvaluator(
        labelCol="is_anomaly", predictionCol="prediction", metricName="f1"
    )
    ...
    logger.info(
        f"Model Evaluation -> Accuracy: {accuracy:.4f}, F1-Score: {f1_score:.4f}"
    )
